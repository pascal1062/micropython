...
class BinaryInput():
    ...
    def reading(self):
        if time.ticks_diff(time.ticks_ms(), self._debounce) > 50:
            self._debounce = time.ticks_ms()
            self._reading = self.volt()
            filtered = self._lastreading + (0.2 * (self._reading - self._lastreading))
            self._lastreading = self._reading
            if filtered <= 1.5: self._newvalue = True
            elif filtered >= 3.5: self._newvalue = False
