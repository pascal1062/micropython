...
class Serial:
    ...
    def __init__(self, uart_id, baudrate=9600, data_bits=8, stop_bits=1, parity=None, tx_pin=17, rx_pin=16, ctrl_pin=None):
        if baudrate <= 19200:
            t15_char = round((1.5 * (11 / baudrate)) * 1000)
            t35_char = round((3.5 * (11 / baudrate)) * 1000)
        else:
            t15_char = 1
            t35_char = 2
        self._uart = UART(uart_id, baudrate=baudrate, bits=data_bits, parity=parity, stop=stop_bits, tx=tx_pin, rx=rx_pin, timeout=t35_char, timeout_char=t15_char)
    ...
    def get_request(self, unit_addr_list, timeout=None):
        ...
        try:
            ...
            return Request(self, req_no_crc)
